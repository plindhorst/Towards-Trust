...
class HelpSeekerAgent(BW4TBrain):
    numberOfTicksWhenReady = None

    def __init__(self, condition=None, slowdown=7):
        ...
        self._followingAdvice = False
        self._condition = "explainable"
    ...
    def decide_on_bw4t_action(self, state: State):
        ...
        while True:
            ...
            if Phase.FIND_NEXT_GOAL == self._phase:
                ...
                if self._goalVic in self._foundVictims and 'location' in self._foundVictimLocs[self._goalVic].keys():
                    ...
                    if self._goalVic in self._uncarryable:
                        if self._condition == "explainable":
                            self._sendMessage(
                                'I need you to pick up ' + self._goalVic + ' in ' + self._foundVictimLocs[self._goalVic][
                                    'room'] + ' because I am not allowed to carry critically injured adults.',
                                'RescueBot')
                        if self._condition == "adaptive":
                            msg1 = 'I need you to pick up ' + self._goalVic + ' in ' + \
                                   self._foundVictimLocs[self._goalVic][
                                       'room'] + ' because I am not allowed to carry critically injured adults.'
                            msg2 = 'I need you to pick up ' + self._goalVic + ' in ' + \
            ...
            if Phase.ADVICE_ROOM == self._phase:
                msg1 = 'I need your advice on which room to search. Pick the room you think I should explore. ' \
                       'Select NO if you do not wish to give me advice. If I don\'t receive any advice, I will explore ' \
                       'the closest (unsearched) room.'
                self._sendMessage(msg1, 'RescueBot')
                self._tick = state['World']['nr_ticks']
                self._previousMessage = self.received_messages[-1]
                self._phase = Phase.WAIT_FOR_ADVICE
                if self._mode == 'normal':
                    return Idle.__name__, {'duration_in_ticks': 50}
                if self._mode == 'quick':
                    return Idle.__name__, {'duration_in_ticks': 10}
            ...
            if Phase.WAIT_FOR_ADVICE == self._phase:
                ...
                if self.received_messages and self.received_messages[-1] != self._previousMessage and self.received_messages[-1] != 'No':
                    ...
                    if areaId in areas:
                        area = "area " + self.received_messages[-1][-2:]
                        self._door = state.get_room_doors(area)[0]
                        self._followingAdvice = True
                        self._phase = Phase.PLAN_PATH_TO_ROOM
                return Idle.__name__, {'duration_in_ticks': 3}
            ...
            if Phase.FOLLOW_PATH_TO_ROOM == self._phase:
                ...
                if self._door['room_name'] in self._searchedRooms and self._goalVic not in self._foundVictims:
                    ...
                    if self._goalVic not in self._foundVictims:
                        if self._condition == "explainable":
                            if self._followingAdvice:
                                self._sendMessage('Moving to ' + str(self._door[
                                                                         'room_name']) + ' to search for ' + self._goalVic + ' and because it is the area you advised me to explore.',
                                                  'RescueBot')
                            else:
                                self._sendMessage('Moving to ' + str(self._door[
                                                                         'room_name']) + ' to search for ' + self._goalVic + ' and because it is the closest unsearched area.',
                                                  'RescueBot')
                            if str(self._door['room_name']) in ["area C3", "area A4", "area B2"]:
                                msg1 = 'This room is big, could you help me search it ? '
                                self._sendMessage(msg1, 'RescueBot')
                        if self._condition == "adaptive":
                        ...
                        if 'class_inheritance' in info and 'CollectableBlock' in info['class_inheritance']:
                            ...
                            if 'healthy' not in vic and vic not in self._foundVictims and 'boy' not in vic and 'girl' not in vic:
                                ...
                                if vic == self._goalVic and vic in self._uncarryable:
                                    if self._condition == "explainable":
                                        self._sendMessage('URGENT: Help me pick up ' + vic + ' in ' + self._door[
                            ...
                            if vic in self._undistinguishable and vic not in self._foundVictims and vic != self._waitedFor:
                                if self._condition == "explainable":
                                    self._sendMessage(
                                        'URGENT: Please help me clarify the gender of the injured baby in ' + self._door[
                                            'room_name'] + ' because I am unable to distinguish them. Please come here and press button "Boy" or "Girl".',
                                        'RescueBot')
                                if self._condition == "adaptive":
                                    msg1 = 'URGENT: I need help clarifying the gender of the injured baby in ' + self._door[
                                        'room_name'] + ' because I am unable to distinguish them. Please come here and press button "Boy" or "Girl".'
                                    msg2 = 'URGENT: I need help clarifying the gender of the injured baby in ' + self._door[
                                        'room_name'] + '. Please come here and press button "Boy" or "Girl".'
                                    explanation = 'because I am unable to distinguish them'
                                    self._dynamicMessage(msg1, msg2, explanation, 'RescueBot')
                                if self._condition == "silent" or self._condition == "transparent":
                                    self._sendMessage(
                                        'URGENT: I need help clarifying the gender of the injured baby in ' + self._door[
                                            'room_name'] + '. Please come here and press button "Boy" or "Girl".',
                                        'RescueBot')
                                self._foundVictim = str(info['img_name'][8:-4])
    ...
    def _processMessages(self, state):
        ...
        for i in range(len(self.received_messages)):
            ...
            if msg.startswith("Search:"):
                ...
                if area not in self._searchedRooms and not self.received_messages[i-1].startswith("I need your advice on which room"):
