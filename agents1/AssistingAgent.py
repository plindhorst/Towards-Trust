...
class AssistingAgent(BW4TBrain):
    ...
    def __init__(self, condition, slowdown: int):
        ...
        AssistingAgent.numberOfTicksWhenReady = self._maxTicks
        self._sendMessages = []
        self._mode = 'normal'
    ...
    def decide_on_bw4t_action(self, state: State):
        ...
        while True:
            ...
            if self.received_messages and self.received_messages[-1].startswith('What are the commands for'):
                self._sendMessage("To pick up a victim press the 'Q' key. To drop off a victim press the 'W' key.", 'RescueBot')
            if self.received_messages and self.received_messages[-1].startswith('What are the injury severity'):
                self._sendMessage("Red is high severity injury. We prioritize these. Yellow is a mild severity injury. " +
            ...
            if Phase.INTRODUCTION == self._phase:
                ...
                if self.received_messages and self.received_messages[-1] == 'Ready!' or not state[
                    ...
                    if AssistingAgent.numberOfTicksWhenReady == self._maxTicks:
                        AssistingAgent.numberOfTicksWhenReady = state['World']['nr_ticks']
